...
draft: false
summary: '如何在 Django 中配置静态文件，搭配 Nginx 在生产环境下进行访问。'
tags: ['guide']
---
...
<Image src={'/static/images/how-to-handle-the-static-files-in-django/a311b0d8gy1fq0dx3gdqsj20p80izdhg.jpg'} width={908} height={603} />

## STATICFILES_DIR

通常静态资源文件，例如一些通用的 *.css、*.js、*.png 等等，可能全部 APP 都要用到，如 bootstrap.min.css，将它在每个 `app/static/app_name/css/` 里面复制粘贴一遍？
...
<Image src={'/static/images/how-to-handle-the-static-files-in-django/a311b0d8gy1fq0eoizv66j20jw0b475d.jpg'} width={716} height={400} />

执行完毕后，在根目录，会新增一个 `static_cdn/`

<Image src={'/static/images/how-to-handle-the-static-files-in-django/a311b0d8gy1fq0epp14evj207s0brjrq.jpg'} width={280} height={423} />

## NGINX

假设使用的 WEB 服务器是 `Nginx`，并且我们已经成功部署了该 `Django` 项目。

由于 `Django` 设置的 `STATIC_UR`L 为 `/static/`，因此，客户端依然会和开发模式时一样，静态资源文件请求，仍然是按照
